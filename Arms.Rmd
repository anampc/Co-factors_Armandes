...
Nutrients_C<-cbind(N.Armand, N.Armanda[,-1 ])
colnames(Nutrients_C) <-c("Beaker", "Armand", "Armanda")

kable(as.data.frame(Nutrients_C, format = "markdown"), 
      longtable = TRUE)
```
...
Duration<-ggarrange(Minutes_nutrients+
                      theme(legend.position = "none"), Minutes_nutrientsb,
                    Minutes_disease2+
                      theme(legend.position = "none"), Minutes_disease2b,
                    labels=c("Nutrients and Temperature ~ 2 weeks", "", 
                             "Nutrients, Temperature and Disease ~ 1 week"), 
                    ncol = 2, nrow = 2, widths = c(2.8,1.2))
Duration
```
...
write.csv(Stats.table, "Dosing regime", row.names = F)
```
